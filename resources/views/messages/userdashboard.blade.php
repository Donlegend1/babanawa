										...
										<table class="table-striped table-hover table">
											<tbody>
												@foreach ($messages as $message)
													<tr class="unread" id="mail_msg_1" onclick="window.location='/message-inbox/{{ $message->message->id }}';"
														...
														<td class="open-view">
															@if ($message->message->status_id === 1)
																<span class="label label-primary mr-2">Approved</span>&nbsp;
															@endif
															@if ($message->message->status_id === 0)
																<span class="label label-primary mr-2">Incomplete Approval</span>&nbsp;
															@endif
															<span class="msgtext">{{ $message->message->subject }}</span>
														</td>
														<td class="open-view"><span
																class="msg_time">{{ Carbon\Carbon::parse($message->message->created_at)->isoFormat('Do MMMM YYYY') }}</span>
														</td>
													</tr>
												@endforeach
											</tbody>
